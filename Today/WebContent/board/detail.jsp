...
<script type="text/javascript">
	...
	function mySubmit(index){
		if(index==1){			//수정버튼 클릭시
			document.detail.action='<%=myctx%>/update.do';
		}//--1번
		if(index==2){			//삭제버튼 클릭시
			var isDel=confirm('정말 삭제하시겠습니까?');
			if(isDel==true){
				document.detail.action='<%=myctx%>/delete.do';
			}
		}//--2번
		if(index==3){			//댓글작성 클릭시
			if(!$('#reply').val()){
			...
			document.detail.action='<%=myctx%>/write_reply.do';
		}//--3번
		...
		<div class="modal-content">
			<form name="detail" method="post">
				...
				<div class="madal-footer text-center">
					<button type="button" class="btn btn-primary" onclick="mySubmit(1)">수정</button>
					<button type="button" class="btn btn-warning" onclick="mySubmit(2)">삭제</button>
